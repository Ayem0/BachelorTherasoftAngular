...
export class LocaleService {
  ...
  public loadLang() {
    ...
    const browserLang = this.toLang(this.translateService.getBrowserLang());
    const langToUse =
      storedLang ||
      browserLang ||
      (this.translateService.getDefaultLang() as Lang);
  ...
  private toTz(tz: string | null | undefined): string | undefined {
    if (tz) {
      return timeZones.includes(tz) ? tz : undefined;
    }
    return undefined;
  }

  private getTzFromLocalStorage(): string | undefined {
    const storedTz = localStorage.getItem(this.tzKey);
    return this.toTz(storedTz);
  }
